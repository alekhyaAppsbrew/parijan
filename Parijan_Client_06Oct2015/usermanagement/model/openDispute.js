  ...
  function openDispute(request,response)
  {
    // Inserting all the request details into inboxData table 
    // Data before sending to the server
    fs.appendFile('log',"\n URL :/openDispute - API NAME :openDispute  METHOD TYPE : POST"+"\nINPUT:\n" ,function()
    {
      fs.appendFile('log',"transactionId:"+request.body.transactionId+"\nremarks:"+request.body.remarks,function()
      {        
           connection.query("UPDATE project.inboxData SET remarks='"+request.body.remarks+"', status='DISPUTE' WHERE transactionId='"+request.body.transactionId+"';",function(err, rows,result)   
           {
             // If there was an error then send false
                if(err)
                {
                  ...
                  fs.appendFile('log',"\nOUTPUT:\n" ,function()
                  {
                    fs.appendFile('log',"response:"+new Boolean(0)+"\n------------------------------------------------------------------------------",function(){console.log("OUTPUT from openDispute Written to log file");}); 
